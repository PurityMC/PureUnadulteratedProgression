...
/**
 * Basic interface for a multiblock machine part. This is defined as an abstract class as we need the basic
 * functionality of a TileEntity as well. Preferably, you should derive from MultiblockTileEntityBase, which does all
...
public abstract class IMultiblockPart extends TileEntity {
    ...
    /**
     * Called when this block is being orphaned. Use this to copy game-data values that should persist despite a machine
     * being broken. This should NOT mark the part as disconnected. onDetached will be called immediately afterwards.
     *
    ...
    /**
     * Factory method. Creates a new multiblock controller and returns it. Does not attach this tile entity to it.
    ...
    /**
     * Retrieve the type of multiblock controller which governs this part. Used to ensure that incompatible multiblocks
     * are not merged.
     *
    ...
    /**
     * Called when this block is moved from its current controller into a new controller. A special case of
     * attach/detach, done here for efficiency to avoid triggering lots of recalculation logic.
     *
    ...
    /**
     * Returns an array containing references to neighboring IMultiblockPart tile entities. Primarily a utility method.
     * Only works after tileentity construction, so it cannot be used in MultiblockControllerBase::attachBlock.
     * <p/>
    ...
    /**
     * Called when a machine is fully assembled from the disassembled state, meaning it was broken by a player/entity
     * action, not by chunk unloads. Note that, for non-square machines, the min/max coordinates may not actually be
     * part of the machine! They form an outer bounding box for the whole machine itself.
     *
    ...
    /**
     * Called when the user activates the machine. This is not called by default, but is included as most machines have
     * this game-logical concept.
     */
    public abstract void onMachineActivated();

    /**
     * Called when the user deactivates the machine. This is not called by default, but is included as most machines
     * have this game-logical concept.
     */
    ...
    /**
     * Called when this part should check its neighbors. This method MUST NOT cause additional chunks to load. ALWAYS
     * check to see if a chunk is loaded before querying for its tile entity This part should inform the controller that
     * it is attaching at this time.
     *
    ...
    /**
     * Assert that this part is detached. If not, log a warning and set the part's controller to null. Do NOT fire the
     * full disconnection logic.
     */
    ...
    /**
     * Called after a block is added and the controller has incorporated the part's saved multiblock game-data into
     * itself. Generally, you should clear the saved data here.
     */
