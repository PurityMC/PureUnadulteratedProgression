...
public class BlockPlateBender extends BlockMachineBase {
    ...
    public BlockPlateBender() {
        super(Material.rock);
        setBlockName("techreborn.platebender");
    }

    @Override
    public TileEntity createNewTileEntity(World p_149915_1_, int p_149915_2_) {
        return new TilePlateBender();
    }

    @Override
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        float hitY, float hitZ) {
        if (!player.isSneaking()) player.openGui(PUP.pupinstance, GuiHandler.plateBenderID, world, x, y, z);
        return true;
    }
