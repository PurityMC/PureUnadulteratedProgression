    ...
    public static void load() {
        ...
        for (PartEnum partVal : PartEnum.values()){
            ...
            for (MaterialEnum mat : MaterialEnum.values())
                if (mat.getMaterial().getParts().contains(partVal.name())) mats.put(mat.ordinal(), mat.material);
            ...
            for (Map.Entry<Integer, Material> mat : mats.entrySet()) {
                OreDictionary.registerOre(partVal.name() + mat.getValue().getName(), new ItemStack(part, 1, mat.getKey()));
                ItemApi.setItem(partVal.name() + mat.getValue().getName(), new ItemStack(part, 1, mat.getKey()));
            }
        }
    }
}
